...
public class ScoreResultManager : MonoBehaviour
{
    ...
    public Vector2[] imageSizes = new Vector2[5] // أحجام الصور من الإنسبكتور
    {
        new Vector2(300, 300),
        new Vector2(300, 300),
        new Vector2(300, 300),
    ...
    public float imageAppearDuration = 1f; // مدة ظهور الصورة
    ...
    void SetupImageSizes()
    {
        // تعيين أحجام الصور من الإنسبكتور
        for (int i = 0; i < resultImages.Length && i < imageSizes.Length; i++)
        {
            if (resultImages[i] != null)
            {
                RectTransform rectTransform = resultImages[i].GetComponent<RectTransform>();
                if (rectTransform != null)
                {
                    rectTransform.sizeDelta = imageSizes[i];
                }
            }
        }
    }
    ...
    IEnumerator ImageAppearAnimation(RawImage image)
    {
        ...
        float timer = 0f;
        float duration = imageAppearDuration;
    ...
    // دالة للمساعدة في تعديل الحجم من الكود إذا احتجت
    public void SetImageSize(int imageIndex, Vector2 newSize)
    {
        if (imageIndex >= 0 && imageIndex < imageSizes.Length)
        {
            imageSizes[imageIndex] = newSize;

            // تحديث الحجم فوراً إذا كانت الصورة موجودة
            if (imageIndex < resultImages.Length && resultImages[imageIndex] != null)
            {
                RectTransform rectTransform = resultImages[imageIndex].GetComponent<RectTransform>();
                if (rectTransform != null)
                {
